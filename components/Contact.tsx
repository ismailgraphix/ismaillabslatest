...
export default function Contact() {
    ...
    return (
        <section id="contact" className="py-28 bg-[var(--app-bg)] relative overflow-hidden">
            ...
            <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-start">
                <div>
                    ...
                    <h2 className="font-heading font-black text-[var(--text)] uppercase leading-tight text-[clamp(1.8rem,3.5vw,2.8rem)] tracking-tight mb-5">
                        LET'S START A DISCUSSION ABOUT YOUR NEXT PROJECT
                    </h2>
                    <p className="font-body text-[var(--muted)] leading-relaxed mb-10">
                ...
                <div className="bg-[var(--surface-2)] border border-[var(--border)] rounded-2xl p-8 shadow-sm">
                    {state === "success" ? (
                        <div className="py-10 text-center">
                            ...
                            <h3 className="font-heading font-black text-2xl text-[var(--text)] uppercase mb-2">Message Sent!</h3>
                            <p className="font-body text-[var(--muted)] mb-6">We'll get back to you within 24 hours.</p>
                            <button onClick={() => setState("idle")} className="font-body text-sm text-[#4353FF] hover:text-[var(--text)] transition-colors">Send another message →</button>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <h3 className="font-heading font-black text-2xl text-[var(--text)] uppercase mb-6">Send us a message</h3>
                            {state === "error" && (
                            ...
                            <div className="grid sm:grid-cols-2 gap-5">
                                {[
                                    ...
                                    <div key={f.key}>
                                        <label className="block font-body font-semibold text-[11px] text-[var(--muted)] uppercase tracking-widest mb-2">{f.label}</label>
                                        <input type={f.type} value={(form as any)[f.key]} onChange={e => setForm({...form,[f.key]:e.target.value})}
                                               placeholder={f.placeholder} required
                                               className="w-full bg-[var(--surface-2)] border border-[var(--border)] rounded-xl text-[var(--text)] font-body text-sm px-4 py-3 placeholder-gray-500 focus:outline-none focus:border-[#4353FF] transition-colors"/>
                                    </div>
                                ))}
                            </div>
                            <div>
                                <label className="block font-body font-semibold text-[11px] text-[var(--muted)] uppercase tracking-widest mb-2">Subject</label>
                                <input type="text" value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})}
                                       placeholder="Project inquiry"
                                       className="w-full bg-[var(--surface-2)] border border-[var(--border)] rounded-xl text-[var(--text)] font-body text-sm px-4 py-3 placeholder-gray-500 focus:outline-none focus:border-[#4353FF] transition-colors"/>
                            </div>
                            <div>
                                <label className="block font-body font-semibold text-[11px] text-[var(--muted)] uppercase tracking-widest mb-2">Message</label>
