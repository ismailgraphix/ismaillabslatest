                            ...
                            <div className="flex flex-wrap gap-2">
                                {STATUS_OPTIONS.map(s => (
                                    <button key={s} onClick={() => updateStatus(selectedClient.id, s)}
                                        className={`font-body text-xs font-semibold uppercase tracking-wider px-3 py-1.5 border capitalize transition-all ${selectedClient.status === s ? STATUS_COLORS[s] : "text-gray-600 border-white/10 hover:border-white/30 hover:text-white"
                                            }`}>{s}</button>
                                ))}
                            </div>
                        </div>
