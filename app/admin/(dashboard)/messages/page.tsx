                    ...
                    <div className="flex gap-1 px-4 py-3 border-b border-white/[0.06]">
                        {["all", "unread", "read", "replied", "archived"].map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`font-body text-[11px] font-semibold uppercase tracking-wider px-2.5 py-1.5 transition-all ${filter === f ? "bg-[#4353FF] text-white" : "text-gray-500 hover:text-white"
                                    }`}>
                                {f}
                            </button>
                        ))}
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {loading ? (
                            ...
                            <button key={msg.id} onClick={() => openMessage(msg)}
                                className={`w-full text-left px-5 py-4 border-b border-white/[0.04] transition-colors group ${selected?.id === msg.id ? "bg-[#4353FF]/10 border-l-2 border-l-[#4353FF]" : "hover:bg-white/[0.02]"
                                    }`}>
