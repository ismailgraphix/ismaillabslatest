...
export default function AdminLogin() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            // ✅ The API now returns a 303 redirect with the cookie attached.
            // fetch follows redirects by default, but we need to NOT follow it
            // so we can detect success vs error ourselves.
            ...
            // 303 redirect = success, cookie is set on that response
            if (res.type === "opaqueredirect" || res.status === 303 || res.status === 0) {
                // Hard navigate — browser will send the cookie it just received
                window.location.href = "/admin/dashboard";
                return;
            }

            // Any 2xx non-redirect also means success (fallback)
            if (res.ok) {
                window.location.href = "/admin/dashboard";
                return;
            }

            // Error response — parse JSON for the message
            try {
