...
export default function AdminLogin() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            // Step 1: POST credentials — server sets httpOnly cookie in response
            const res = await fetch("/api/admin/login", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(form),
                credentials: "same-origin", // ensures Set-Cookie is stored by browser
            });
            ...
            // Step 2: Verify cookie was actually stored before navigating.
            // Poll /api/admin/check (which reads the httpOnly cookie server-side).
            // This bridges the gap between "cookie set in response" and "cookie available in next request".
            let verified = false;
            for (let i = 0; i < 5; i++) {
            ...
            if (!verified) {
                // Cookie didn't persist — likely a browser/SameSite issue in dev
                // Fall back: try direct navigation anyway
                console.warn("Cookie check failed, navigating anyway");
            }

            // Step 3: Hard navigate — browser sends the stored cookie,
            // middleware reads it, grants access to dashboard
            ...
            <div className="hidden lg:flex lg:w-[52%] relative flex-col justify-between p-14 overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div className={`mt-10 space-y-3 transition-all duration-700 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
                        {["Permission-based access control","Client & project management","Team management & roles","Full audit trail"].map((f, i) => (
                ...
                <div className={`relative z-10 flex gap-10 pt-8 border-t border-white/10 transition-all duration-700 delay-500 ${mounted ? "opacity-100" : "opacity-0"}`}>
                    {[{num:"4",label:"Role Levels"},{num:"25+",label:"Permissions"},{num:"∞",label:"Clients"}].map(s => (
                        <div key={s.label}>
            ...
            <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
                ...
                <div className={`w-full max-w-[420px] transition-all duration-700 delay-100 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <button type="button" onClick={() => setShowPass(!showPass)}
                                    ...
                                    <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
                                        {showPass
                                            ? <><path d="M2 8s2.5-4.5 6-4.5S14 8 14 8s-2.5 4.5-6 4.5S2 8 2 8z" stroke="currentColor" strokeWidth="1.4"/><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.4"/><path d="M3 3l10 10" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></>
                                            : <><path d="M2 8s2.5-4.5 6-4.5S14 8 14 8s-2.5 4.5-6 4.5S2 8 2 8z" stroke="currentColor" strokeWidth="1.4"/><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.4"/></>
                                        }
                                    </svg>
                                </button>
                            </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#4353FF] text-white font-heading font-black py-4 uppercase tracking-wider text-sm hover:bg-white hover:text-[#0f0f0f] transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed relative overflow-hidden group mt-2"
                        >
                ...
                {loading ? (
                    <>
                        ...
                        {error ? "Signing in…" : "Verifying session…"}
                    </>
                ) : (
                    <>Sign In <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7h8M8 4l3 3-3 3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></>
                )}
              </span>
                        </button>
                    </form>
                    ...
                    <div className="mt-8 pt-6 border-t border-white/5 text-center space-y-3">
                        <p className="font-body text-gray-600 text-xs">
                            First time?{" "}
                            <a href="/admin/setup" className="text-[#4353FF] hover:text-white transition-colors font-semibold">Create super admin →</a>
                        </p>
                        <a href="/" className="font-body text-gray-700 text-xs hover:text-gray-400 transition-colors block">← Back to main site</a>
                    </div>
                </div>
            </div>
        </div>
